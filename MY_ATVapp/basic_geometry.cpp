...
MyMesh createCube() {
	...
	float* buffer = (float*)malloc(sizeof(vertices) + sizeof(normals) + sizeof(texCoords) + sizeof(tangents));
	memcpy(buffer, vertices, sizeof(vertices));
	memcpy(buffer + sizeof(vertices)/sizeof(float), normals, sizeof(normals));
	memcpy(buffer + (sizeof(vertices) + sizeof(normals))/sizeof(float), texCoords, sizeof(texCoords));
	memcpy(buffer + (sizeof(vertices) + sizeof(normals) + sizeof(texCoords))/sizeof(float), tangents, sizeof(tangents));

	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices)+sizeof(normals)+sizeof(texCoords)+sizeof(tangents),buffer,GL_STATIC_DRAW);
	free(buffer);
